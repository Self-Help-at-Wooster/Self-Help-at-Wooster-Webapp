...
function SubmitSlip(UUID, SlipType, Text, P1, P2, P3, P4, R, CitPer){
    ...
    && typeof CitPer === 'string' && CitPer.charAt(0) !== "="){
      ...
      var greight = true;
      if(UserData[studentData["ACCESS"]-1] == accessLevels["CAPTAIN"] && UserData[studentData["GRADE"]-1] < 11 && SlipType == 2){
      greight = false;
      }
  
      if(UserData[studentData["ACCESS"]-1] >= accessLevels["ADMIN"] && UserData[studentData["ACCESS"]-1] <= accessLevels["CAPTAIN"] && greight == true){
...
function writeParameterLog(Row){
  //Utilities.sleep(10);
