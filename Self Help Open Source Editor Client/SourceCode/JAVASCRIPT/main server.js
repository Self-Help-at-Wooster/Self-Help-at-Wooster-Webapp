...
/**
 * Gets the User Email or returns -1 if it is null
 * @returns String or -1 corresponding to above
 */
function getEmail() {
 ...
 * @returns Array of User Data, -1 if not found
 */
...
//Provide ColumnNum (with enum data) and value you are searching for
 ...
 * @param x Left Component
 * @param y Right Component 
 * @returns Integer Comparison
 */
...
function writeLog(log){
  ...
  if(ScriptID != "1n3epqcZQ6cQPNXbkhZ0erPxWVbRKcgiJmom8dljUr6bEYrUhCCCHXzKr"
    ...
    if(log && log.charAt(0) === "=")//remove any equals signs
    log = log.replace("0", "");
    ...
    var time = new Date(); 
    var targetRange = cursheet.getRange(2, 1, 1, 4).setValues([[Session.getActiveUser().getEmail(), time.toLocaleTimeString(), (time.getMonth() + 1) + "/" + time.getDate() + "/" + time.getYear(), log]]);
